...
GetOptions('in|i=s'=>\$infile,'vcf|v=s'=>\$vcf_file,'tsv|t=s'=>\$tsv_file,'case|c=s'=>\$case)|| warn "perl prepare_bionano_output.pl --in IN --vcf VCF --tsv TSV --case g\n";
chomp ($infile,$vcf_file,$tsv_file,$case);
...
if($case eq "s"){# somatic case, the raw infile has no parents information
  # print header into TSV file, vcf file does not need headers
  print TSV "coords\talt_chrom_trans\ttype\tconfidence\tSVFreq\tSVSize\tOverlapGenes\tnonOverlapgene\tNonOveGenedist\tself_mole_found\n";
  foreach my $infile_line (@in_csv_file){
        chomp $infile_line;
        if($infile_line=~/#/){
              # header
              ...
              my$full_line_things_needed="$coords_done\t$lineparts[3]\t$lineparts[9]\t$lineparts[8]\t$lineparts[22]\t$lineparts[26]\t$lineparts[30]\t$lineparts[31]\t$lineparts[32]\t$lineparts[35]\n";
              #$full_line_things_needed=~s/\t\w/\t/;
              $full_line_things_needed=~s/\n\s/\n/;
              $full_line_things_needed=~s/\r//;
              $full_line_things_needed=~s/\n.*/\n/;
